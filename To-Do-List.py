...
class TodoListApp(tk.Tk):
    def __init__(self):
        ...
        ttk.Button(self, text="Add", command=self.add_task).pack(side=tk.TOP, pady=5)
        ...
        # Create buttons for marking tasks as done, deleting them, and updating tasks
        ttk.Button(self, text="Done", style="success.TButton",
                   command=self.mark_done).pack(side=tk.LEFT, padx=5, pady=10)
        ttk.Button(self, text="Delete", style="danger.TButton",
                   command=self.delete_task).pack(side=tk.LEFT, padx=5, pady=10)
        ttk.Button(self, text="Update", command=self.update_task).pack(side=tk.LEFT, padx=5, pady=10)
    ...
    def update_task(self):
        task_index = self.task_list.curselection()
        if task_index:
            updated_task = simpledialog.askstring("Update Task", "Enter updated task:")
            if updated_task is not None:
                self.task_list.delete(task_index)
                self.task_list.insert(tk.END, updated_task)
                self.task_list.itemconfig(tk.END, fg="orange")
                self.save_tasks()
